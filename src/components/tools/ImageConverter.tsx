            ...
            <div className="space-y-6">
                  <Card>
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-4">
                              {FORMATS.map((f) => (
                                    <Button
                                          key={f.value}
                                          variant={targetFormat === f.value ? 'primary' : 'ghost'}
                                          size="sm"
                  ...
                  {files.length > 0 && (
                        <div className="flex gap-3 justify-end">
                              <Button onClick={handleConvert} loading={isProcessing} icon={<Repeat className="w-5 h-5" />}>
                                    Convert {files.length} File{files.length !== 1 ? 's' : ''}
                              </Button>
                              {files.some((f) => f.processedFile) && (
                                    <Button variant="secondary" onClick={() => {
                                          const ext = FORMATS.find((f) => f.value === targetFormat)?.ext || 'jpg';
                                          files.filter((f) => f.processedFile).forEach((f) =>
                                                downloadBlob(f.processedFile!, f.name.replace(/\.[^.]+$/, `.${ext}`))
                                          );
                                    }} icon={<Download className="w-5 h-5" />}>Download All</Button>
                              )}
                        </div>
                  )}
            </div>
