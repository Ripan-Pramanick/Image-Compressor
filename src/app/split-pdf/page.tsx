'use client';

import React, { useState, useCallback } from 'react';
import { ToolLayout } from '@/components/ui/ToolLayout';
import { DropZone } from '@/components/ui/DropZone';
import { Button } from '@/components/ui/Button';
import { Card } from '@/components/ui/Card';
import { ProgressBar } from '@/components/ui/ProgressBar';
import { useToast } from '@/hooks/useToast';
import { useCompressionHistory } from '@/hooks/useCompressionHistory';
import { splitPDF, extractPDFPages, deletePDFPages } from '@/lib/pdf-processing';
import { downloadBlob } from '@/lib/utils';
import { Scissors, Download, Copy, Trash2 } from 'lucide-react';

export default function SplitPdfPage() {
        const [sourceFile, setSourceFile] = useState<File | null>(null);
        const [mode, setMode] = useState<'split' | 'extract' | 'delete'>('split');
        const [pageRanges, setPageRanges] = useState<string>('');
        const [results, setResults] = useState<{ filename: string; blob: Blob }[]>([]);
        const [isProcessing, setIsProcessing] = useState(false);

        const { addToHistory } = useCompressionHistory();
        const { showSuccess, showError } = useToast();

        const handleFileSelect = useCallback((files: File[]) => {
                if (files.length > 0) {
                        setSourceFile(files[0]);
                        setResults([]);
                }
        }, []);

        const handleProcess = useCallback(async () => {
                if (!sourceFile) return;

                setIsProcessing(true);
                try {
                        switch (mode) {
                                case 'split': {
                                        const ranges = pageRanges
                                                .split(',')
                                                .map((r) => r.trim())
                                                .filter(Boolean)
                                                .map((r) => {
                                                        const [start, end] = r.split('-').map(Number);
                                                        return { start, end: end || start };
                                                });

                                        const splitResults = await splitPDF(sourceFile, ranges);
                                        setResults(
                                                splitResults.map((r) => ({
                                                        filename: r.filename,
                                                        blob: new Blob([new Uint8Array(r.data) as unknown as BlobPart], { type: 'application/pdf' }),
                                                }))
                                        );
                                        break;
                                }

                                case 'extract': {
                                        const pages = pageRanges
                                                .split(',')
                                                .map((p) => parseInt(p.trim()))
                                                .filter((p) => !isNaN(p));

                                        const extracted = await extractPDFPages(sourceFile, pages);
                                        setResults([
                                                {
                                                        filename: `extracted_pages.pdf`,
                                                        blob: new Blob([new Uint8Array(extracted)], { type: 'application/pdf' }),
                                                },
                                        ]);
                                        break;
                                }

                                case 'delete': {
                                        const pagesToDelete = pageRanges
                                                .split(',')
                                                .map((p) => parseInt(p.trim()))
                                                .filter((p) => !isNaN(p));

                                        const result = await deletePDFPages(sourceFile, pagesToDelete);
                                        setResults([
                                                {
                                                        filename: `cleaned_${sourceFile.name}`,
                                                        blob: new Blob([new Uint8Array(result)], { type: 'application/pdf' }),
                                                },
                                        ]);
                                        break;
                                }
                        }

                        addToHistory({
                                toolId: 'split-pdf',
                                toolName: 'Split PDF',
                                filesProcessed: 1,
                                spaceSaved: 0,
                                timestamp: Date.now(),
                        });

                        showSuccess(`PDF processed successfully`);
                } catch (error) {
                        showError('Failed to process PDF');
                } finally {
                        setIsProcessing(false);
                }
        }, [sourceFile, mode, pageRanges, addToHistory, showSuccess, showError]);

        const handleDownload = useCallback(
                (blob: Blob, filename: string) => {
                        downloadBlob(blob, filename);
                        showSuccess(`Downloaded ${filename}`);
                },
                [showSuccess]
        );

        const handleDownloadAll = useCallback(() => {
                results.forEach((r) => downloadBlob(r.blob, r.filename));
                showSuccess(`Downloaded ${results.length} file(s)`);
        }, [results, showSuccess]);

        return (
                <ToolLayout
                        title="Split PDF"
                        description="Split PDF files, extract specific pages, or delete unwanted pages"
                >
                        <div className="space-y-6">
                                <Card>
                                        <h3 className="font-semibold text-gray-900 dark:text-white mb-4">
                                                Operation Mode
                                        </h3>
                                        <div className="flex gap-2 mb-4">
                                                {[
                                                        { value: 'split', label: 'Split by Range', icon: <Scissors className="w-4 h-4" /> },
                                                        { value: 'extract', label: 'Extract Pages', icon: <Copy className="w-4 h-4" /> },
                                                        { value: 'delete', label: 'Delete Pages', icon: <Trash2 className="w-4 h-4" /> },
                                                ].map((m) => (
                                                        <Button
                                                                key={m.value}
                                                                variant={mode === m.value ? 'primary' : 'ghost'}
                                                                size="sm"
                                                                onClick={() => setMode(m.value as typeof mode)}
                                                                icon={m.icon}
                                                        >
                                                                {m.label}
                                                        </Button>
                                                ))}
                                        </div>

                                        <div>
                                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                                                        {mode === 'split'
                                                                ? 'Page ranges (e.g., 1-3, 5, 7-9)'
                                                                : mode === 'extract'
                                                                        ? 'Pages to extract (e.g., 1, 3, 5-7)'
                                                                        : 'Pages to delete (e.g., 2, 4, 6)'}
                                                </label>
                                                <input
                                                        type="text"
                                                        value={pageRanges}
                                                        onChange={(e) => setPageRanges(e.target.value)}
                                                        placeholder="e.g., 1-3, 5, 7-9"
                                                        className="w-full px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800"
                                                />
                                        </div>
                                </Card>

                                {!sourceFile ? (
                                        <DropZone
                                                onFilesSelected={handleFileSelect}
                                                acceptedTypes={{ 'application/pdf': ['.pdf'] }}
                                                maxFiles={1}
                                        />
                                ) : (
                                        <Card>
                                                <div className="flex items-center justify-between">
                                                        <div>
                                                                <p className="font-medium text-gray-900 dark:text-white">
                                                                        {sourceFile.name}
                                                                </p>
                                                                <p className="text-sm text-gray-500">
                                                                        {(sourceFile.size / (1024 * 1024)).toFixed(2)} MB
                                                                </p>
                                                        </div>
                                                        <Button
                                                                variant="ghost"
                                                                size="sm"
                                                                onClick={() => {
                                                                        setSourceFile(null);
                                                                        setResults([]);
                                                                }}
                                                        >
                                                                Change File
                                                        </Button>
                                                </div>
                                        </Card>
                                )}

                                {results.length > 0 && (
                                        <Card>
                                                <h3 className="font-semibold text-gray-900 dark:text-white mb-4">
                                                        Results ({results.length} file{results.length !== 1 ? 's' : ''})
                                                </h3>
                                                <div className="space-y-2">
                                                        {results.map((result, i) => (
                                                                <div
                                                                        key={i}
                                                                        className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg"
                                                                >
                                                                        <span className="text-sm font-medium text-gray-900 dark:text-white">
                                                                                {result.filename}
                                                                        </span>
                                                                        <Button
                                                                                variant="ghost"
                                                                                size="sm"
                                                                                onClick={() => handleDownload(result.blob, result.filename)}
                                                                                icon={<Download className="w-4 h-4" />}
                                                                        >
                                                                                Download
                                                                        </Button>
                                                                </div>
                                                        ))}
                                                </div>
                                                {results.length > 1 && (
                                                        <Button
                                                                variant="secondary"
                                                                onClick={handleDownloadAll}
                                                                icon={<Download className="w-5 h-5" />}
                                                                className="mt-4"
                                                        >
                                                                Download All
                                                        </Button>
                                                )}
                                        </Card>
                                )}

                                {isProcessing && <ProgressBar indeterminate label="Processing PDF..." />}

                                {sourceFile && (
                                        <div className="flex justify-end">
                                                <Button
                                                        onClick={handleProcess}
                                                        disabled={isProcessing || !pageRanges.trim()}
                                                        loading={isProcessing}
                                                        icon={<Scissors className="w-5 h-5" />}
                                                >
                                                        Process PDF
                                                </Button>
                                        </div>
                                )}
                        </div>
                </ToolLayout>
        );
}